 ...
 *
 * It was hinged twice before and torn both times. A rectangle at the elbow took
 * the hand and the revolver and left the sleeve behind. An outline traced round
 * the armpit, turning about a point twenty pixels inside the body, tore worse:
 * the seam it was cut along ran up the front of the forearm, forty pixels from
 * the point it turned about, so every degree of the swing opened it. Lowered,
 * the sleeve stopped in a flat cut at the wrist and the glove hung below with
 * nothing joining the two - the forearm, which is the part of him a player
 * looks straight at.
 *
 * A cut-out arm cannot be hinged on a straight seam at all. Rotating about a
 * point takes every straight line through it to a different line, so a straight
 * seam opens by exactly the angle of the swing, and a seam that misses the
 * point opens further still. The one curve a rotation leaves where it found it
 * is a circle about the point it turns on. So the joint is a disc: everything
 * within it belongs to the body and never moves, everything beyond it on the
 * forearm's side is the arm and turns, and the disc's edge meets itself at
 * every angle. The forearm comes out from under a cuff that is always drawn.
 *
...
/* ---- the flourish ---- *
 * He has one drawing and the revolver is already in his hand in it, so the gun
 * cannot literally leave the leather and come back: there is no holstered gun
 * anywhere in the artwork, and an empty holster with no gun to explain it reads
 * as a bin. What can be done is the spin itself.
 *
 * A road agent's spin turns the gun on the trigger finger. The finger is the
 * pin: the revolver goes round it and the hand stays where it is - which is
 * both what the trick actually is and, here, the only version of it that can
 * be drawn, because turning the whole fist about the wrist swings forty-five
 * pixels of glove and barrel round an arc and reads as a gun coming off its
 * owner rather than spinning in his grip.
 *
 * So it is hinged the way the forearm is hinged on the elbow, one joint
 * further out and much smaller: a disc on the trigger guard that the rotation
 * cannot move off itself, with the revolver beyond it. Nested inside the arm's
 * own turn, so the spin happens wherever the arm is pointing.
 *
 * It only ever happens during a draw or a holster. Settled at either end the
 * spin is nothing and the arm is the arm, which is what keeps the levelled
 * pose exact to the pixel. */
const GUNPOLY=[[84,104],[84,86],[100,80],[132,80],[132,102],[106,110]];
const TRIGGER={x:97,y:106}, TJOINT=9;
let spin=0;                           // radians, the hand about the wrist
/* A draw is not a dial being turned. It is over before it has been watched,
 * and the way it reads as fast is that it arrives early and settles late: the
 * arm is most of the way up in the first third, overshoots level, and comes
 * back onto it. Putting it away is the other shape - it gives, then drops.
 ...
 * settling lerp got a tick in first, and a jump at the start of a draw is the
 * one frame a player is certain to be looking at. */
function beginSeq(kind,now){seqKind=kind;seqAt=now;seqFrom=swing;}
function seqNow(now){
  if(!seqKind)return null;
  const span=seqKind==="draw"?DRAW_MS:PUTUP_MS;
  /* The frame clock is the timestamp rAF was called with, which is the start
   * of the frame and so can be earlier than the keypress that began this. A
   * negative t there is not "not started yet", it is "started, and this frame
   * is the first of it" - read as null it hands the frame back to the settling
   * lerp, which is a jump on the one frame a player is certain to see. */
  const t=Math.max(0,(now-seqAt)/span);
  if(t>=1){seqKind="";return null;}
  if(seqKind==="draw"){
    // up fast, past level, and back onto it
    const rise=1-Math.pow(1-t,3);
    const over=0.13*Math.sin(t*Math.PI)*(1-t*0.55);
    // and round once, finished before the arm has settled
    const u=Math.min(1,t/0.62);
    return {swing:seqFrom+(1-seqFrom)*rise+over,
            spin:-TURN*(1-Math.pow(1-u,2))};
  }
  // putting it away: it gives at the top, then goes
  const fall=Math.pow(1-t,2.2);
  const u=Math.max(0,Math.min(1,(t-0.12)/0.78));
  return {swing:seqFrom*fall, spin:TURN*(1-Math.pow(1-u,2))};
}
...
/* The arm's reach: the ground the forearm, the glove and the revolver cover.
 * Everything in it is theirs; nothing else in the drawing is inside it. */
function reachPath(dx){
  ctx.moveTo(dx+ARMPOLY[0][0],ARMPOLY[0][1]);
  for(let i=1;i<ARMPOLY.length;i++)ctx.lineTo(dx+ARMPOLY[i][0],ARMPOLY[i][1]);
  ctx.closePath();
}
function jointPath(dx){
  ctx.moveTo(dx+ELBOW.x+JOINT,ELBOW.y);
  ctx.arc(dx+ELBOW.x,ELBOW.y,JOINT,0,Math.PI*2);
}
/* Canvas clips intersect and cannot subtract, so "outside the joint" is the
 * whole picture and the joint together, counted odd-even. */
function beyondJointPath(dx){
  ctx.rect(dx-OWN.w,OWN.y-OWN.h,OWN.w*3,OWN.h*3);
  jointPath(dx);
}
/* and the same three shapes again, one joint further out */
function gunPath(dx){
  ctx.moveTo(dx+GUNPOLY[0][0],GUNPOLY[0][1]);
  for(let i=1;i<GUNPOLY.length;i++)ctx.lineTo(dx+GUNPOLY[i][0],GUNPOLY[i][1]);
  ctx.closePath();
}
function pinPath(dx){
  ctx.moveTo(dx+TRIGGER.x+TJOINT,TRIGGER.y);
  ctx.arc(dx+TRIGGER.x,TRIGGER.y,TJOINT,0,Math.PI*2);
}
function beyondPinPath(dx){
  ctx.rect(dx-OWN.w,OWN.y-OWN.h,OWN.w*3,OWN.h*3);
  pinPath(dx);
}
function beyondGunPath(dx){
  ctx.rect(dx-OWN.w,OWN.y-OWN.h,OWN.w*3,OWN.h*3);
  gunPath(dx);
}
function ownGun(out){
  ...
  const turning=Math.abs(spin)>0.004;
  const paint=()=>ctx.drawImage(img,dx,OWN.y,OWN.w,OWN.h);
  const arm=()=>{                     // the arm's own turn, about the elbow
    if(Math.abs(a)>0.001){
      ctx.translate(dx+ELBOW.x,ELBOW.y); ctx.rotate(a);
      ctx.translate(-(dx+ELBOW.x),-ELBOW.y);
    }
  };
  // 1. him, with the arm's whole reach cut out of him
  ctx.save();
  ctx.beginPath();
  ...
  paint();
  ctx.restore();
  // 2. and the joint put back, unturned: the elbow and the cuff are his, not
  //    the arm's, and they are drawn in the same place at every angle. This is
  //    what the forearm comes out from under, and why nothing can tear.
  ctx.save();
  ctx.beginPath(); reachPath(dx); ctx.clip();
  ctx.beginPath(); jointPath(dx);  ctx.clip();
  paint();
  ctx.restore();
  // 3. the arm: the same drawing turned about the elbow, clipped to the reach
  //    and to what lies beyond the joint - both built after the turn, so the
  //    cut turns with it
  ctx.save();
  arm();
  ctx.beginPath(); reachPath(dx); ctx.clip();
  ctx.beginPath(); beyondJointPath(dx); ctx.clip("evenodd");
  if(turning){                        // the revolver is being drawn separately
    ctx.beginPath(); beyondGunPath(dx); ctx.clip("evenodd");
  }
  paint();
  ctx.restore();
  if(turning){
    // 4. the trigger guard put back, unspun: the pin the gun turns on
    ctx.save();
    arm();
    ctx.beginPath(); reachPath(dx); ctx.clip();
    ctx.beginPath(); gunPath(dx); ctx.clip();
    ctx.beginPath(); pinPath(dx); ctx.clip();
    paint();
    ctx.restore();
    // 5. and the revolver, turned on that pin inside the arm's own turn, so
    //    the spin goes wherever the arm is pointing
    ctx.save();
    arm();
    ctx.translate(dx+TRIGGER.x,TRIGGER.y); ctx.rotate(spin);
    ctx.translate(-(dx+TRIGGER.x),-TRIGGER.y);
    ctx.beginPath(); reachPath(dx); ctx.clip();
    ctx.beginPath(); gunPath(dx); ctx.clip();
    ctx.beginPath(); beyondPinPath(dx); ctx.clip("evenodd");
    paint();
    ctx.restore();
  }
  // the flare belongs on the muzzle wherever the arm has put it, and never
  // while he is still spinning it
  if(k>0.25&&!turning){ctx.save(); arm(); muzzle(dx,a,k); ctx.restore();}
}
 ...
 * frame arrives there is no frame time, and the wall will do. */
const clock=()=>lastNow>=0?lastNow:performance.now();
function frame(now){
  try{
    ...
    const seq=seqNow(now);
    if(seq){swing=seq.swing; spin=seq.spin;}
    else{
      spin=0;
