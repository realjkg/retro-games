...
function visitor(enc,pose,now){
  ...
  const bdx=sway+mood.lean+(walk?walk.dx:0), bdy=breath+mood.rise+(walk?walk.bob:0);
  ctx.fillStyle="rgba(0,0,0,.35)";                   // his shadow goes with him
  ctx.fillRect(SPRX+4*FIGCW+(walk?walk.dx:0),SPRY+(lowest+1)*FIGCH,16*FIGCW,FIGCH);
  ...
  if(walk){
    ...
    drawFigure(rows.map((r,i)=>i>=knee?r:""),SPRX+bdx+walk.boot,SPRY+bdy,look);
