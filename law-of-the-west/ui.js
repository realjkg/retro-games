...
function visitor(enc,pose,now){
  ...
  const wob=walk?(1-Math.abs(Math.cos(walk.t*Math.PI*STRIDE))):0;
  const wbob=walk?-Math.round(wob*1.6):0;
  const wlean=walk?Math.round((1-walk.t)*2)*GEST:0;
  const legX=(walk?walk.dx:0);
  ...
  // legs: they carry the weight and, walking, the stride
  drawFigure(band(hips,rows.length),
    SPRX+legX+hipX+(walk?walk.boot*GEST:0),SPRY+(walk?wbob:0),look);
